...
do -- Set properties
    ...
    objects["Instance92"]["Text"] = "发送远程事件";
    ...
    objects["Instance96"]["Text"] = "接收远程事件";
...
return function(shared, page)
    ...
    addButton("阻止远程", function()
        if selection[1] then
            block[selection[1]] = true
        end
    end)
    addButton("远程数据块 [名称]", function()
        if selection[1] then
            block[selection[1].Name] = true
        end
    end)
    
    addButton("清除日志", function()
        for _, log in logs do
            log:Destroy()
        end
